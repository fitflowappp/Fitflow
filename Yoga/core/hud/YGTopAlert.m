...
-(void)addAlertImgv{
    ...
    self.alertImgv.center = CGPointMake(16*SCALE+self.alertImgv.frame.size.width/2,29*SCALE+20);
    [self addSubview:self.alertImgv];
    
}

-(void)addAlertLabel{
    CGFloat marginX = CGRectGetMaxX(self.alertImgv.frame)+16*SCALE;
    self.alertLabel = [[UILabel alloc] initWithFrame:CGRectMake(marginX,20,self.frame.size.width-marginX-16*SCALE,self.frame.size.height-20)];
    ...
    self.alertLabel.font = [UIFont fontWithName:@"Lato-Regular" size:14*SCALE];
...
+(YGTopAlert*)alert:(NSString*)msg bkColorCode:(NSString*)code{
    ...
    YGTopAlert *alert = [[YGTopAlert alloc] initWithFrame:CGRectMake(0,-58*SCALE-20,GET_SCREEN_WIDTH,58*SCALE+20)];
...
-(void)show{
    [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:1 initialSpringVelocity:20 options:UIViewAnimationOptionLayoutSubviews animations:^{
        self.frame = CGRectMake(0,0,GET_SCREEN_WIDTH,58*SCALE+20);
...
-(void)hide{
    [UIView animateWithDuration:0.5 delay:3 usingSpringWithDamping:1 initialSpringVelocity:20 options:UIViewAnimationOptionLayoutSubviews animations:^{
        self.frame = CGRectMake(0,-58*SCALE-20,GET_SCREEN_WIDTH,58*SCALE+20);
    } completion:^(BOOL finished) {
